...
export class ReportController {
  ...
  async assignProjectToReport(
    @Body() data: { reportId: string; projectId: string },
  ) {
  ...
  async assignUserToReport(
    @Body() data: { reportId: string; userId: string },
  ): Promise<Report> {
  ...
  async downloadExcel(@Param('id') id: string, @Res() res: Response) {
    const { fileName, stream } = await this.reportService.downloadReportExcel(
      id,
    );
    res.setHeader('Content-Disposition', `attachment; filename=${fileName}`);
    res.setHeader(
